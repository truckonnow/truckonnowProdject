    ...
    public class AskDelyveryMV : BindableBase
    {
        ...
        private async Task CheckVechicleAndGoToResultPage()
        {
            ...
            if (vehiclwInformation1s.Count - 1 == indexCurrentVechecle)
            {
                ...
                await Navigation.PushAsync(new ClientStart(managerDispatchMob, IdShip, initDasbordDelegate, OnDeliveryToCarrier, TotalPaymentToCarrier, VehiclwInformation, getShiping, getVechicleDelegate));
            }
            else
            {
