...
namespace MDispatch.Service
{
    public class A_R
    {
        ...
        internal int RequestPasswordChanges(string login, string password, ref string description)
        {
            IRestResponse response = null;
            string content = null;
            try
            {
                RestClient client = new RestClient(Config.BaseReqvesteUrl);
                RestRequest request = new RestRequest("Mobile/RequestPassword", Method.POST);
                client.Timeout = 20000;
                request.AddParameter("email", login);
                request.AddParameter("fullName", password);
                response = client.Execute(request);
                content = response.Content;
            }
            catch (Exception)
